...
class PoisonedDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, idx_attacker, epsilon, trigger_size, polarity, pos, target_label, timesteps, dataname, mode, num_classes, split=False, begin_t=0, end_t=0):
        super().__init__()
        ...
        if split:
            self.data, self.targets = add_perturbation_split(dataset, idx_attacker, epsilon, trigger_size, polarity, pos, target_label, timesteps, dataname, mode, begin_t, end_t)
        else:
            self.data, self.targets = add_perturbation(dataset, idx_attacker, epsilon, trigger_size, polarity, pos, target_label, timesteps, dataname, mode)
...
def process(dataset, dataname, time_step, mode):
    ...
    if type(dataset) == torch.utils.data.Subset:
        ...
        data = np.array([np.array(x[0])
                                for x in dataset])
        
    return data, targets

def add_perturbation_split(dataset, idx_attacker, epsilon, trigger_size, polarity, pos, target_label, timesteps, dataname, mode, t_begin, t_end):
    """
...
def add_perturbation(dataset, idx_attacker, epsilon, trigger_size, polarity, pos, target_label, timesteps, dataname, mode):
    """
    Add perturbation to the dataset
    :param dataset: dataset to be poisoned
    :param epsilon: poisoning rate
