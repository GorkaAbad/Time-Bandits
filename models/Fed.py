    ...
    def FedAvg(self, w, w_init=None):
        ...
        for k in w_avg.keys():
            if w_init:
                w_avg[k] = (
                    w_avg[k].cpu()
                    + torch.mean(torch.abs(w_init[k].cpu() - w[0][k].cpu()) * 1.0)
                    * torch.randn(w[0][k].size())
                    * self.args.grad_noise_stdev
                )  # Scale the noise by mean of the absolute value of the model updates
            else:
                w_avg[k] = (
                    w_avg[k].cpu()
                    + torch.randn(w[0][k].size()) * self.args.grad_noise_stdev
                )  # Add gaussian noise to the model updates
        for k in w_avg.keys():
            for i in range(1, len(w)):
                if non_stragglers[i] == 1:
                    if w_init:
                        w_avg[k] = (
                            ...
                            + w[i][k].cpu()
                            + torch.mean(
                                torch.abs(w_init[k].cpu() - w[i][k].cpu()) * 1.0
                            )
                            * torch.randn(w[i][k].size())
                            * self.args.grad_noise_stdev
                        )  # Scale the noise by mean of the absolute value of the model updates
                        ...
                        w_avg[k] = (
                            w_avg[k].cpu()
                            + w[i][k].cpu()
                            + torch.randn(w[i][k].size()) * self.args.grad_noise_stdev
                        )  # Add gaussian noise to the model updates
        ...
        for k in w_init.keys():
            ...
            if th_basis == "magnitude" and pruning_type == "uniform":
                ...
                if activity_mask is None:
                    print(
                        "Activity mask is not available. Activity based pruning not possible"
                    )
                if "features" in k:
                    idx = int(k.split(sep=".")[2])
                ...
                print(
                    "Unknown threshold basis or pruning_type. Available options: th_basis - magnitude or activity, pruning_type - uniform or dynamic"
                )
        ...
        for k in w_init.keys():
            for i in range(1, len(delta_w_locals)):
                ...
                if th_basis == "magnitude" and pruning_type == "uniform":
                    ...
                    if activity is None:
                        print(
                            "Layer activity not available. Dynamic sparsity not possible"
                        )
                    if "features" in k:
                        idx = int(k.split(sep=".")[2])
                        layer_activity = activity[idx]
